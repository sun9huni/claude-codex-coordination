...
|---|---|
| "I'll create a page before checking whether one already exists." | Search/fetch first to avoid duplicate durable records. |
| "The user mentioned it, so it is safe to record as fact." | Separate facts, decisions, rationale, and open questions; mark uncertainty explicitly. |
| "I'll skip database properties and just paste markdown." | Notion pages need owner, status, date, tags, and relations when the database requires them. |
| "I'll keep private or transient chat details because they might help later." | Capture reusable knowledge only; avoid private, stale, or unverified context. |

## Forbidden

- Do NOT write to Notion before the target page/database is clear.
- Do NOT create duplicate pages when an existing page should be updated.
- Do NOT omit required database properties.
- Do NOT present a Notion write as complete if the MCP call failed.
